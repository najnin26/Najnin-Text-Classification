...
## Abstract
The natural language processing field requires sentiment analysis and text classification as essential functions, yet deep learning models from the past struggle to handle complex feature interactions with their existing modeling methods. To address this limitation, this paper proposes **QDeBERTa-BiLSTM**, a solution that employs a hybrid quantum-classical framework that integrates DeBERTa-based contextual embeddings, BiLSTM sequence modeling, and variational quantum circuits to enhance feature representation. The suggested model employs quantum circuits to represent nonlinear interactions while maintaining high contextual knowledge via transformer architectures. Multiple experiments are executed on benchmark datasets such as Spam, Movie Reviews (MR), and MPQA. The model outperforms several previous hybrid and classical baselines, achieving **97.13\%** accuracy on Spam and around **86\%** accuracy on both MR and MPQA. Furthermore, various quantum circuit designs are tested, revealing that circuit design has a major impact on performance. The results highlight the effectiveness of combining classical and quantum techniques for improved text classification.
</HighlightedSection>
